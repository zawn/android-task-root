...
public class TraceFormatter {
	...
	public static void main(String[] args) {
		...
		processLog(traceFile);

	}

	public static void processLog(String logdir) {
		if (logdir == null) {
			usageAndExit();
		}
		String traceFile = logdir;
		...
		if (file.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				File file2 = files[i];
				try {
					TraceFormatter tf = new TraceFormatter(file2.getAbsolutePath(), file2.getAbsolutePath() + ".html");
					tf.format();
				} catch (IOException e) {
					e.printStackTrace();
					System.exit(1);
			...
			try {
				TraceFormatter tf = new TraceFormatter(traceFile, traceFile + ".html");
				tf.format();
			} catch (IOException e) {
				e.printStackTrace();
				System.exit(1);
	...
	public void format() throws IOException {
		...
		while (tp != null) {
			...
			if (traceProperties.getProperty(tp.id + ".value") != null) {
				out.println("  <td>" + traceProperties.getProperty(tp.id + ".class") + ":"
						+ traceProperties.getProperty(tp.id + ".line") + "</td>");
				out.println("  <td>" + MessageFormat.format(traceProperties.getProperty(tp.id + ".value"), tp.inserts)
						+ "</td>");
	...
	public TracePoint read() {
		/*
		 * public long timestamp; public byte type; public short id; public byte level; public String threadName; public Throwable throwable;
		 */
